	...
	const dtuple_t*	entry)	/*!< in: index entry */
{
	...
	if (!row_search_index_entry(entry, BTR_PURGE_TREE, &pcur, &mtr)) {
	...
	const dtuple_t*	entry)	/*!< in: index entry */
{
	...
	if (index->is_spatial()) {
		...
		found = row_search_index_entry(entry, mode, &pcur, &mtr);
		index->lock.u_unlock();
	} else {
		found = row_search_index_entry(entry, mode, &pcur, &mtr);
	}
...
func_exit:
	mtr.commit();
cleanup:
	btr_pcur_close(&pcur); // FIXME: when is btr_cur->rtr_info set?
	return success;
}
